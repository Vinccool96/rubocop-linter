...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.debug = exports.getInput = void 0;
const core = __importStar(__nccwpck_require__(2186));
function getInput(name, parseAsBool = false) {
    if (parseAsBool) {
...
const baseDir = path_1.default.join(process.cwd(), (0, io_1.getInput)("workdir") || "");
(0, io_1.debug)(baseDir);
const git = (0, simple_git_1.default)({ baseDir });
core.info(`Running in ${baseDir}`);
function execute() {
    return __awaiter(this, void 0, void 0, function* () {
        const branchesInfo = yield git.branch();
        (0, io_1.debug)(branchesInfo);
        const currentBranch = branchesInfo.branches[branchesInfo.current];
        const currentCommit = currentBranch.commit;
        (0, io_1.debug)(yield (0, child_process_1.exec)(`git diff ${currentCommit}~ ${currentCommit} --name-only`), `git diff ${currentCommit}~ ${currentCommit} --name-only`);
        const diff = yield git.diff([`${currentCommit}`, currentCommit, "--name-only"]);
        (0, io_1.debug)(diff);
        const files = (0, util_1.prefilterFiles)(diff.split("\n").filter((file) => file), baseDir);
        (0, io_1.debug)(files);
        if (files.length) {
            yield (0, child_process_1.exec)(`cd ${baseDir}`);
            if (!(0, io_1.getInput)("skip_install", true)) {
                processInstall();
            }
            const bundleExec = (0, io_1.getInput)("use_bundler", true) ? "bundle exec " : "";
            const filesString = !(0, io_1.getInput)("all_files", true) ? "" : (0, util_1.filesToString)((0, util_1.filterFiles)(files, baseDir));
            core.startGroup("Running rubocop...");
            const fails = yield (0, util_1.promisifyExec)(`${bundleExec}rubocop --autocorrect --fail-level ${(0, io_1.getInput)("fail_level")} ${(0, io_1.getInput)("rubocop_flags")} ${filesString}`)
                .then(() => true)
                .catch(() => (0, io_1.getInput)("fail_on_error", true));
            core.endGroup();
            if (fails) {
...
function processInstall() {
    ...
    let rubocopVersion;
    if ((0, io_1.getInput)("rubocop_version") === "gemfile") {
        rubocopVersion = (0, util_1.getRubocopVersionFromGemfile)(baseDir);
    }
    else {
        rubocopVersion = (0, io_1.getInput)("rubocop_version");
    }
    (0, child_process_1.exec)(`gem install -N rubocop --version "${rubocopVersion}"`);
    const extensions = (0, io_1.getInput)("rubocop_extensions").split(" ").filter((s) => s);
    for (const extension of extensions) {
        ...
        if (inputExtensionVersion === "gemfile") {
            extensionVersion = (0, util_1.getVersionFromGemfile)(baseDir, extensionName);
        }
        const extensionFlag = extensionVersion ? `--version ${extensionVersion}` : "";
        (0, child_process_1.exec)(`gem install -N "${extensionName}" ${extensionFlag}`);
    }
    core.endGroup();
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.filesToString = exports.filterFiles = exports.prefilterFiles = exports.getVersionFromGemfile = exports.getRubocopVersionFromGemfile = exports.promisifyExec = void 0;
const fs_1 = __importDefault(__nccwpck_require__(7147));
const child_process_1 = __nccwpck_require__(2081);
const core = __importStar(__nccwpck_require__(2186));
const yaml_1 = __nccwpck_require__(4083);
function promisifyExec(command) {
    return new Promise((resolve, reject) => (0, child_process_1.exec)(command, (error, stdout, _stderr) => {
        if (error) {
            if (error.code === 1) {
                // leaks present
                reject(stdout);
...
exports.promisifyExec = promisifyExec;
function getRubocopVersionFromGemfile(filePath) {
    let version = "";
    if (fs_1.default.existsSync(`${filePath}/Gemfile.lock`)) {
        fs_1.default.readFile(`${filePath}/Gemfile.lock`, "utf8", function (_err, data) {
...
exports.getRubocopVersionFromGemfile = getRubocopVersionFromGemfile;
function getVersionFromGemfile(filePath, gem) {
    let version = "";
    if (fs_1.default.existsSync(`${filePath}/Gemfile.lock`)) {
        fs_1.default.readFile(`${filePath}/Gemfile.lock`, "utf8", function (_err, data) {
...
exports.getVersionFromGemfile = getVersionFromGemfile;
function prefilterFiles(files, baseDir) {
    return baseDir !== "." ? files.filter((file) => new RegExp(`^${baseDir}/`, "g").test(file)) : files;
}
exports.prefilterFiles = prefilterFiles;
function filterFiles(files, baseDir) {
    var _a;
    let filteredFiles = files;
    if (baseDir !== ".") {
        filteredFiles = filteredFiles.filter((file) => new RegExp(`^${baseDir}/`, "g").test(file));
    }
    if (fs_1.default.existsSync(`${baseDir}/.rubocop.yml`)) {
        const rubocopConfigFile = fs_1.default.readFileSync(`${baseDir}/.rubocop.yml`, "utf8");
        const config = (0, yaml_1.parse)(rubocopConfigFile);
        const excluded = (_a = config.AllCops) === null || _a === void 0 ? void 0 : _a.Exclude;
        if (excluded) {
